...
function maxMinRevenue(attributes, column1, conditions1, column2, conditions2, date){
	...
	.then (function(result){
		...
		for (var i=0;i<result.length;i++){
			...
			// var a=Date.parse(result[i].closeDate);
			// var b=Date.parse(date);
			// console.log(result[i].closeDate, date, a>b);
			if (Date.parse(result[i].closeDate)>=Date.parse(date)){
				// console.log('in the date');
				deals[j]={
					name:result[i].name,
