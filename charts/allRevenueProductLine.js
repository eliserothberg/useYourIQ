    ...
    var categories = [];
     ...
     var bar = yield generate('bar', options, {
  	...
  	setTimeout(clearFile, 300000);
		...
		for (var n = 0; n < passEmp.length; n++) {
    	passEmp[n].total = passEmp[n].total.toLocaleString();
    	passEmp[n].label = passEmp[n].label.split('(')[0];
  	}
...
function allRevenueProductLine(title, dataName, newVersusReturning, startDate, endDate, res, thisSymbol){
...
module.exports=allRevenueProductLine;
